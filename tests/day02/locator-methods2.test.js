...
test("innerText method test", async ({ page }) => {
  ...
  const paragraph = await page.locator(
    "//p[contains(text(),'Also known as split testing')] "
  );
...
test("inputValue method test: only works with <input> or <textarea> or <select>", async ({
  page,
}) => {
  // go to https://practice.cydeo.com/inputs
  await page.goto("https://practice.cydeo.com/inputs");

  // locate inputbox with //input[@type='number']
...
test("getAttribute method test: can get the value of an attribute of the element", async ({ page }) => {
  ...
  console.log(`Element link: ${elementLink}`);
